...
def plot_altair_multiline_highlight(data, x, y, **kwargs):
    ...
    - warning_status : {'always','ignore'}. Steuerung der Warnmeldungen.
    - palette_scheme (str): Farbpalette für die Linien (Altair-Scheme, z.B. 'tableau10', 'viridis', 'plasma'). 
      Default: 'tableau10'.
    - custom_palette (list[str] | None): Eigene Farbpalette als Liste von Hex-/CSS-Farben (z.B. ['#003f5c','#58508d','#bc5090']). 
        Wenn gesetzt, hat sie Vorrang vor `palette_scheme`. Default: None.      
    - line_width (float): Standard-Strichbreite der Linien. 
      Beim Hervorheben wird sie automatisch verdoppelt. Default: 1.2.
    - x_sort : list | None.
        Explizite Sortierfolge der X-Achse.
        Beispiel: x_sort=[20,21,22,23,24,1,2,3,4,5,6]
        Wenn None, wird Altairs Standardsortierung verwendet.        
    ...
    try:
        ...
        x_sort = kwargs.get('x_sort', None)
        palette_scheme = kwargs.get('palette_scheme', 'tableau10')
        custom_palette = kwargs.get("custom_palette", None)
        line_width = kwargs.get('line_width', 1.2)

        highlight = alt.selection_point(on='pointerover', fields=[category], nearest=True)

        if x_sort is None:
            x_enc = alt.X(x, axis=alt.Axis(title=x_beschriftung))
        else:
            x_enc = alt.X(x, sort=x_sort, axis=alt.Axis(title=x_beschriftung))
        ...
        if category == '':
            ...
            base = alt.Chart(data, title=myTitle).encode(
                ...
                color=alt.Color(
                    category,
                    legend=alt.Legend(title=category_beschriftung, orient="right"),
                    scale=(
                        alt.Scale(scheme=palette_scheme)
                        if custom_palette is None
                        else alt.Scale(range=custom_palette)
                    )
                ),
        ...
        lines = base.mark_line().encode(
            strokeWidth=alt.condition(
                ~highlight,
                alt.value(line_width),
                alt.value(line_width * 2)
            )
        )        
...
def plot_sns_facetgrid(data, col, hue, col_wrap, height, x, y, **kwargs):
    ...
    - myTitle (str): Der Titel des Diagramms.
    - palette (str | list[str] | dict[level,str]): Explizite Farbpalette.
    - hue_order (list): Reihenfolge der Hue-Kategorien
    ...
    try:
        ...
        # Optional: Whitespace in hue-Werten entschärfen (hilft oft)
        data = data.copy()
        data[hue] = data[hue].astype(str).str.strip()
        ...
        # FIX: map_dataframe statt map
        fg.map_dataframe(grafiktyp, x=x, y=y, alpha=0.8)

        fg.set_axis_labels(xlabel, ylabel)
        fg.set_titles(col_template="{col_name}", row_template="{row_name}")

        # FIX: Legend sauber erzeugen
        fg.add_legend(title=hue)

        fg.fig.subplots_adjust(top=0.90)
        fg.fig.suptitle(myTitle, y=1.02)
